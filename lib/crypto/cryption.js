...
class Cryption {
  ...
  _encrypt(rawString) {
    const cipher = createCipheriv(this.AES_ALGORITHM, this.hashKey, this.hashIV);
    const encrypted = cipher.update(rawString, 'bin', 'hex');
    return encrypted + cipher.final('hex');
  }

  _decrypt(encrypted) {
    const decipher = createDecipheriv(this.AES_ALGORITHM, this.hashKey, this.hashIV);
  ...
  validateSHA(encryptedString, SHA) {
    ...
    if (!isValid) {
      throw new Error('SecurityError: SHA256 does not match. Data received may be tampered with');
    }
    return isValid;
  }

  decrypt(encryptedString) {
    const decrypted = this._decrypt(encryptedString).replace(/[\x00-\x20]/g, '');
    return decrypted;
  }
}
